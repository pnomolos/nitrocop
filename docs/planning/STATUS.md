...
- **ProjectIndexHelp** (5 new cops): recommend leaving unimplemented (08-project-index-scope.md). Upstream defers to the external `rubydex` gem behind `AllCops/UseProjectIndex` (default off); the corpus baseline never enables it, so the oracle cannot score these cops. Revisit when baseline_rubocop.yml + corpus Gemfile gain rubydex and real configs adopt it.
